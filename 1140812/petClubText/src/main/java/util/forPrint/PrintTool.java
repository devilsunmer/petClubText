...
public class PrintTool {
	...
	public static void ifToButton() {
	    int option = JOptionPane.showOptionDialog(
	            ...
	            null,
	            new String[]{"列印 Excel 報表", "列印 TXT 報表", "取消"},
	            "列印 Excel 報表"
	    );
	    ...
	    try {
	        switch (option) {
	            case 0: // Excel
	            	PrintTool.chooseAndPrintReport(); // 已寫好的函式
	                break;
	            case 1: // TXT
	            	PrintTool.chooseAndPrintTxt(); // 下面會提供這個方法
	                break;
	            default:
	                JOptionPane.showMessageDialog(null, "操作已取消");
	        }
	    } catch (Exception e) {
	        JOptionPane.showMessageDialog(null, "列印失敗：" + e.getMessage());
	        e.printStackTrace();
	    }
	}
