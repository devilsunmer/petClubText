...
import util.forPrint.ExcelCreat;
...
public class StringTool {
	...
	public static void useTitleNameForReport(String reportName, String fileName, String sheetName) {
		...
		if (!fileName.toLowerCase().endsWith(".xls")) {
			fileName = "reporter/" + fileName + ".xls";
		} else {
			fileName = "reporter/" + fileName;
		}
		File file = new File(fileName);
		String[] title=StringTool.getTitleNamesForReport(reportName);
		excelCreate.create(fileName, sheetName, title);
		if (file.exists()) {
			switch (reportName) {
			case "顧客消費訂單":
				List<CustOrder> orderList = new CustOrderServiceImpl().viewOrder();
				CustOrder custOrder = new CustOrder();
				for (CustOrder allList : orderList) {
					excelCreate.insertCustOrderMoneyValue(allList, fileName, sheetName);
				}
				break;
			case "寵物員工薪資":
				List<StaffIncome> staffIncomeList = new StaffIncomeDaoImpl().selectAll();
				StaffIncome staffMoney = new StaffIncome();
				for (StaffIncome allList : staffIncomeList) {
					excelCreate.insertStaffWorkValue(allList, fileName, sheetName);
				}
				break;
			case "會員資料":
				List<CustMember> memberList = new CustMemberServiceImpl().allMember();
				CustMember member = new CustMember();
				for (CustMember allList : memberList) {
					excelCreate.insertCustMemberValue(allList, fileName, sheetName);
				}
				break;
			case "潛在顧客資料":
				List<CustFreeMember> custFreeMemberList = new CustFreeMemberServiceImpl().allFreeMember();
				CustFreeMember custFreeMember = new CustFreeMember();
				for (CustFreeMember allList : custFreeMemberList) {
					excelCreate.insertCustFreeMemberValue(allList, fileName, sheetName);
				}
				break;
			case "寵物員工資料":
				List<StaffMember> staffList = new StaffMemberServiceImpl().viewStaff();
				StaffMember staff = new StaffMember();
				for (StaffMember allList : staffList) {
					excelCreate.insertStaffMemberValue(allList, fileName, sheetName);
				}
				break;
			default:
				JOptionPane.showMessageDialog(null, "報表未成功", "報表提示", JOptionPane.INFORMATION_MESSAGE);
				break;
			}
			JOptionPane.showMessageDialog(null, fileName + " 報表已匯出", "匯出完成", JOptionPane.INFORMATION_MESSAGE);
		} else {
			JOptionPane.showMessageDialog(null, fileName + " 報表失敗", "匯出錯誤", JOptionPane.INFORMATION_MESSAGE);
		}
	}
