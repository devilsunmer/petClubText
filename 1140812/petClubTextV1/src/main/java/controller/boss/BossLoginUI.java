...
//		System.out.println(Arrays.toString(StringTool.excelList()));
		...
		printButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					PrintTool.ifToButton(); // 使用統一印表邏輯
				} catch (Exception ex) {
					ex.printStackTrace();
					JOptionPane.showMessageDialog(null, "列印過程發生錯誤：" + ex.getMessage());
				}
			}
		});
		...
		enterMoction.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (text.equals("確認匯出")) {
					...
					ExcelCreat creat=new ExcelCreat();
					String folder = "reporter";
				    File dir = new File(folder);
				    if (!dir.exists()) dir.mkdirs();

				    String fileName = ExcelFieldName;
				    if (!fileName.toLowerCase().endsWith(".xls")) {
				        fileName += ".xls";
				    }
				    fileName = folder + "/" + fileName;

				    creat.create(fileName, ExcelSheetName, StringTool.getTitleNamesForReport(selectedReport));
				    StringTool.useTitleNameForReport(selectedReport, fileName, ExcelSheetName);
				    ...
				    try {
				        File file = new File("reporter", selectedReport);
				        if (!file.exists()) {
